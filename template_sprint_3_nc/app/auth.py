...
def activate():
    try:
        ...
        if request.method == 'GET': 
            ...
            db = get_db()
            attempt = db.execute(
                'SELECT id FROM user WHERE email = ?', (number, utils.U_UNCONFIRMED)
            ).fetchone()

            if attempt is not None:
                db.execute(
                    'SELECT id FROM user WHERE email = ?', (utils.U_CONFIRMED, attempt['id'])
                )
                db.execute(
                    'SELECT id FROM user WHERE username=?', (attempt['username'], attempt['password'], attempt['salt'], attempt['email'])
                )
                db.commit()
    ...
    try:
        ...
        if request.method =='POST': 
            ...
            if not password1:
                ...
                return render_template('auth/register.html', number=authid)
            ...
            db = get_db()
            attempt = db.execute(
                'SELECT id FROM user WHERE email = ?', (authid, utils.F_ACTIVE)
            ).fetchone()
            
            if attempt is not None:
                db.execute(
                    'SELECT id FROM user WHERE email = ?', (utils.F_INACTIVE, attempt['id'])
                )
                salt = hex(random.getrandbits(128))[2:]
                hashP = generate_password_hash(password + salt)   
                db.execute(
                    'SELECT id FROM user WHERE email = ?', (hashP, salt, attempt['userid'])
                )
                db.commit()
                return redirect(url_for('auth.login'))
            else:
...
def change():
    try:
        ...
        if request.method == 'GET': 
            ...
            db = get_db()
            attempt = db.execute(
                'SELECT id FROM user WHERE email = ?', (number, utils.F_ACTIVE)
            ).fetchone()
...
def load_logged_in_user():
    ...
    if user_id is None:
        ...
        g.user = get_db().execute(
            'SELECT id FROM user WHERE email = ?', (user_id,)
        ).fetchone()
